...
export function PersonalInsurancePage() {
  const { t } = useLanguage();
  const category = t.individualSolutions.categories.personalInsurance;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-green-50/30 to-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-12 relative">
          ...
          <h1 className="text-green-600 mb-4 text-5xl font-bold">{category.title}</h1>
          <p className="text-2xl text-gray-700 leading-relaxed max-w-4xl">{category.description}</p>
        </div>
        ...
        <div className="relative w-full aspect-[21/9] rounded-2xl overflow-hidden mb-16 shadow-2xl group">
          ...
          <ImageWithFallback
            src="https://images.unsplash.com/photo-1758691463331-2ac00e6f676f?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxmYW1pbHklMjBoZWFsdGglMjBpbnN1cmFuY2V8ZW58MXx8fHwxNzYyNzc4ODUzfDA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral"
            alt={category.title}
              ...
              if (trimmedLine.startsWith('•')) {
                return (
                  <div key={index} className="flex gap-4 py-2 group/item">
                    <span className="text-green-600 mt-1 transform group-hover/item:scale-125 transition-transform">•</span>
                    <span className="flex-1">{trimmedLine.substring(1).trim()}</span>
                  </div>
                );
              }
              
              return <p key={index} className="text-lg leading-relaxed">{trimmedLine}</p>;
            })}
          </div>
